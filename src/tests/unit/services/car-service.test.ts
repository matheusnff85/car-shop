...
import { carMock, carMockWithId, carChangeMock, carChangeMockWithId } from '../../mocks/carMock';
...
describe('Car Service', () => {
  ...
  before(() => {
    ...
    sinon.stub(carModel, 'update')
      .onCall(0).resolves(carChangeMockWithId)
      .onCall(1).resolves(null);
  });
  ...
  describe('update a car', () => {
    it('on sucess', async () => {
      const updatedCar = await carService.update(carChangeMockWithId._id, carChangeMock);
